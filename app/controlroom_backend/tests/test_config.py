...
async def test_list_lookup(client, slug):
    response = await client.get(f"/studio/config/{slug}")
...
async def test_list_lookup_fields(client, slug):
    item = (await client.get(f"/studio/config/{slug}")).json()[0]
...
async def test_get_lookup(client, conn):
    ...
    response = await client.get(f"/studio/config/tag-types/{row['type_id']}")
    assert response.status_code == 200
    assert response.json()["type_id"] == str(row["type_id"])


async def test_get_lookup_not_found(client):
    response = await client.get(f"/studio/config/tag-types/{uuid4()}")
    assert response.status_code == 404


async def test_unknown_slug(client):
    response = await client.get("/studio/config/nonexistent-table")
    assert response.status_code == 404


async def test_create_lookup(client, admin_headers):
    response = await client.post("/studio/config/tag-types", json={"type_name": "Test Tag Type"}, headers=admin_headers)
...
async def test_create_lookup_minimal(client, admin_headers):
    response = await client.post("/studio/config/effect-types", json={"type_name": "Test Effect Type"}, headers=admin_headers)
    assert response.status_code == 201


async def test_create_lookup_missing_name(client, admin_headers):
    response = await client.post("/studio/config/tag-types", json={"type_description": "No name"}, headers=admin_headers)
...
async def test_update_lookup(client, conn, admin_headers):
    ...
    response = await client.patch(
        f"/studio/config/tool-types/{row['type_id']}",
...
async def test_update_lookup_not_found(client, admin_headers):
    response = await client.patch(f"/studio/config/tag-types/{uuid4()}", json={"type_name": "Ghost"}, headers=admin_headers)
    assert response.status_code == 404
...
async def test_delete_lookup(client, conn, admin_headers):
    ...
    response = await client.delete(f"/studio/config/tag-types/{row['type_id']}", headers=admin_headers)
    assert response.status_code == 204


async def test_delete_lookup_not_found(client, admin_headers):
    response = await client.delete(f"/studio/config/tag-types/{uuid4()}", headers=admin_headers)
    assert response.status_code == 404
