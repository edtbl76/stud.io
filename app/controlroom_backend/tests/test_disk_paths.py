...
Tests cover: effects, instruments, workflow_tools, libraries.
Workstations and remaining tool tables follow the same pattern and are
verified by the shared assertions below.
"""
...
async def test_effect_response_includes_disk_paths(client):
    data = (await client.get("/studio/session/effects")).json()
...
async def test_patch_effect_disk_paths_audited(client, conn, admin_headers):
    ...
    new_data = audit["new_data"]  # asyncpg auto-decodes jsonb
    assert "disk_paths" in new_data
...
async def test_instrument_response_includes_disk_paths(client):
    data = (await client.get("/studio/session/instruments")).json()
    assert "disk_paths" in data["items"][0]


async def test_patch_instrument_disk_paths(client, conn, admin_headers):
    row = await conn.fetchrow(
        "INSERT INTO instruments (instrument_name) VALUES ('DPTest Inst') RETURNING instrument_id"
    )
    resp = await client.patch(
        f"/studio/session/instruments/{row['instrument_id']}",
        json={"disk_paths": [_VALID_ENTRY]},
        headers=admin_headers,
    )
    assert resp.status_code == 200
...
async def test_patch_instrument_disk_paths_invalid_format(client, conn, admin_headers):
    row = await conn.fetchrow(
...
# ---------------------------------------------------------------------------
# Workflow Tools (representative of the 5 tool tables)
# ---------------------------------------------------------------------------

async def test_workflow_tool_response_includes_disk_paths(client):
    data = (await client.get("/studio/tools/workflow")).json()
    assert "disk_paths" in data["items"][0]


async def test_patch_workflow_tool_disk_paths(client, conn, admin_headers):
    row = await conn.fetchrow(
        "INSERT INTO workflow_tools (tool_name) VALUES ('DPTest WF') RETURNING workflow_tool_id"
    )
    resp = await client.patch(
        f"/studio/tools/workflow/{row['workflow_tool_id']}",
        json={"disk_paths": [_VALID_ENTRY]},
        headers=admin_headers,
    )
    assert resp.status_code == 200
...
async def test_library_response_includes_disk_paths(client):
    data = (await client.get("/studio/session/libraries")).json()
    assert "disk_paths" in data["items"][0]


async def test_patch_library_disk_paths(client, conn, admin_headers):
    row = await conn.fetchrow(
        "INSERT INTO libraries (library_name) VALUES ('DPTest Lib') RETURNING library_id"
    )
    resp = await client.patch(
        f"/studio/session/libraries/{row['library_id']}",
        json={"disk_paths": [_VALID_ENTRY]},
        headers=admin_headers,
    )
    assert resp.status_code == 200
...
async def test_patch_library_disk_paths_invalid_format(client, conn, admin_headers):
    row = await conn.fetchrow(
...
async def test_workstation_response_includes_disk_paths(client):
    data = (await client.get("/studio/session/workstations")).json()
    assert "disk_paths" in data["items"][0]


async def test_patch_workstation_disk_paths(client, conn, admin_headers):
    row = await conn.fetchrow(
        "INSERT INTO workstations (tool_name) VALUES ('DPTest WS') RETURNING workstation_id"
    )
    resp = await client.patch(
        f"/studio/session/workstations/{row['workstation_id']}",
        json={"disk_paths": [_VALID_ENTRY]},
        headers=admin_headers,
    )
    assert resp.status_code == 200
