...
async def test_list_instruments_returns_results(client, conn):
    ...
    response = await client.get("/studio/session/instruments")
...
async def test_list_instruments_fields(client, conn):
    ...
    item = (await client.get("/studio/session/instruments")).json()["items"][0]
...
async def test_list_instruments_search_no_match(client):
    response = await client.get("/studio/session/instruments?filter_name=zzznomatchzzz")
    assert response.status_code == 200
...
async def test_filter_instruments_by_brand_prefix_in_name(client, conn):
    ...
    response = await client.get("/studio/session/instruments?filter_name=NameBrand_Inst")
    assert response.status_code == 200
...
async def test_filter_instruments_by_model_brand_name(client, conn):
    ...
    response = await client.get("/studio/session/instruments?filter_models=BrandFilter_Inst")
    assert response.status_code == 200
...
async def test_list_instruments_pagination(client, conn):
    ...
    response = await client.get("/studio/session/instruments?limit=1&offset=0")
    assert response.status_code == 200
    data = response.json()
...
async def test_get_instrument(client, conn):
    ...
    response = await client.get(f"/studio/session/instruments/{row['instrument_id']}")
    assert response.status_code == 200
    assert response.json()["instrument_id"] == str(row["instrument_id"])


async def test_get_instrument_not_found(client):
    response = await client.get(f"/studio/session/instruments/{uuid4()}")
...
async def test_create_instrument(client, admin_headers):
    response = await client.post("/studio/session/instruments", json={"instrument_name": "Test Synth"}, headers=admin_headers)
...
async def test_create_instrument_missing_name(client, admin_headers):
    response = await client.post("/studio/session/instruments", json={"version": "1.0"}, headers=admin_headers)
...
async def test_create_instrument_with_types(client, conn, admin_headers):
    ...
    response = await client.post("/studio/session/instruments", json={
...
@pytest.mark.parametrize("case", [
    _ParentSourceCase(
        ...
        "instrument_id", "instruments", "Child Synth", "Parent Synth",
    ),
    _ParentSourceCase(
        "INSERT INTO workstations (tool_name) VALUES ('Test DAW') RETURNING workstation_id",
        "workstation_id", "workstations", "DAW Instrument", "Test DAW",
    ),
])
async def test_create_instrument_parent_name_resolves(client, conn, admin_headers, case):
    parent = await conn.fetchrow(case.insert_sql)
    response = await client.post("/studio/session/instruments", json={
        "instrument_name": case.child_name,
...
async def test_update_instrument(client, conn, admin_headers):
    ...
    response = await client.patch(f"/studio/session/instruments/{row['instrument_id']}", json={"version": "2.0"}, headers=admin_headers)
    assert response.status_code == 200
...
async def test_update_instrument_parent_ids(client, conn, admin_headers):
    ...
    response = await client.patch(
        f"/studio/session/instruments/{child['instrument_id']}",
...
async def test_update_instrument_not_found(client, admin_headers):
    response = await client.patch(f"/studio/session/instruments/{uuid4()}", json={"version": "2.0"}, headers=admin_headers)
    assert response.status_code == 404
...
async def test_delete_instrument(client, conn, admin_headers):
    ...
    response = await client.delete(f"/studio/session/instruments/{row['instrument_id']}", headers=admin_headers)
    assert response.status_code == 204


async def test_delete_instrument_not_found(client, admin_headers):
    response = await client.delete(f"/studio/session/instruments/{uuid4()}", headers=admin_headers)
    assert response.status_code == 404
...
async def test_filter_instruments_parents(client, conn, admin_headers, case):
    ...
    resp = await client.post("/studio/session/instruments", json={
        "instrument_name": f"ChildInst{case.suffix}",
        "parent_ids": [{"table_name": "instruments", "id": str(parent["instrument_id"])}],
    }, headers=admin_headers)
    assert resp.status_code == 201
    response = await client.get(f"/studio/session/instruments?filter_parents_op={case.op}&limit=9999")
    assert response.status_code == 200
    names = [i["instrument_name"] for i in response.json()["items"]]
