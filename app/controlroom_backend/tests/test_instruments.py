...
async def test_create_instrument(client):
    response = await client.post("/instruments", json={"instrument_name": "Test Synth"})
...
async def test_create_instrument_missing_name(client):
    response = await client.post("/instruments", json={"version": "1.0"})
    assert response.status_code == 422


async def test_create_instrument_with_types(client, conn):
...
async def test_create_instrument_with_parent(client, conn):
...
async def test_update_instrument(client, conn):
    row = await conn.fetchrow(
        "INSERT INTO instruments (instrument_name) VALUES ('Update Me') RETURNING instrument_id"
    )
    response = await client.patch(f"/instruments/{row['instrument_id']}", json={"version": "2.0"})
    assert response.status_code == 200
    assert response.json()["version"] == "2.0"
    assert response.json()["instrument_name"] == "Update Me"


async def test_update_instrument_not_found(client):
    response = await client.patch(f"/instruments/{uuid4()}", json={"version": "2.0"})
    assert response.status_code == 404
...
async def test_delete_instrument(client, conn):
    row = await conn.fetchrow(
        "INSERT INTO instruments (instrument_name) VALUES ('Delete Me') RETURNING instrument_id"
    )
    response = await client.delete(f"/instruments/{row['instrument_id']}")
    assert response.status_code == 204


async def test_delete_instrument_not_found(client):
    response = await client.delete(f"/instruments/{uuid4()}")
    assert response.status_code == 404
