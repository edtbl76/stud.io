...
def _find_index_node(nodes, index_name: str, relation: str) -> bool:
    """Walk EXPLAIN JSON plan nodes recursively; return True if a node uses the named index."""
    for node in nodes:
        plan = node.get("Plan") or node
        if (
            plan.get("Index Name") == index_name
            and plan.get("Relation Name") == relation
            and plan.get("Node Type") in ("Index Scan", "Index Only Scan", "Bitmap Index Scan")
        ):
            return True
        for key in ("Plans", "Plan"):
            child = plan.get(key)
            if isinstance(child, list) and _find_index_node(child, index_name, relation):
                return True
            if isinstance(child, dict) and _find_index_node([child], index_name, relation):
                return True
    return False
...
async def test_vendor_rules_join_uses_index(conn):
    ...
    assert _find_index_node(plan, "idx_vendor_rules_disk_vendor", "scanner_vendor_rules"), \
        "vendor rule JOIN must use Index Scan on idx_vendor_rules_disk_vendor"
...
async def test_name_rules_join_uses_index(conn):
    ...
    assert _find_index_node(plan, "idx_name_rules_disk_name", "scanner_name_rules"), \
        "name rule JOIN must use Index Scan on idx_name_rules_disk_name"
