...
async def _insert_effect(conn, name="Reverb Pro"):
    return await conn.fetchval(
        "INSERT INTO effects (effect_name) VALUES ($1) RETURNING effect_id", name
    )


async def _insert_vendor_rule(conn, disk_vendor="ikmultimedia", enabled=True) -> uuid4:
    return await conn.fetchval(
        "INSERT INTO scanner_vendor_rules (disk_vendor, catalog_vendor, enabled, created_by) "
        "VALUES ($1,$2,$3,$4) RETURNING rule_id",
        disk_vendor, "IK Multimedia", enabled, "admin",
    )
...
async def test_get_rules_returns_all_types(client, conn, admin_headers):
    resp = await client.get("/scanner/rules", headers=admin_headers)
    assert resp.status_code == 200
    data = resp.json()
    assert "vendor" in data and "name" in data and "pattern" in data
...
async def test_create_vendor_rule_returns_rule_and_counts(client, conn, admin_headers):
    ...
    assert "affected_count" in data and "clean_count" in data and "needs_review_count" in data
...
async def test_vendor_rule_duplicate_returns_409(client, conn, admin_headers):
    await _insert_vendor_rule(conn)
    resp = await client.post(
        "/scanner/rules/vendor",
...
@pytest.mark.asyncio
async def test_update_vendor_rule_404(client, admin_headers):
    resp = await client.patch(
...
async def test_delete_vendor_rule(client, conn, admin_headers):
    rule_id = await _insert_vendor_rule(conn)
    resp = await client.delete(f"/scanner/rules/vendor/{rule_id}", headers=admin_headers)
    assert resp.status_code == 204
    assert await conn.fetchrow("SELECT 1 FROM scanner_vendor_rules WHERE rule_id=$1", rule_id) is None
...
async def test_toggle_vendor_rule_disables(client, conn, admin_headers):
    rule_id = await _insert_vendor_rule(conn, enabled=True)
    resp = await client.patch(f"/scanner/rules/vendor/{rule_id}/toggle", json={"enabled": False}, headers=admin_headers)
    assert resp.status_code == 200
...
async def test_toggle_vendor_rule_enables(client, conn, admin_headers):
    rule_id = await _insert_vendor_rule(conn, enabled=False)
    resp = await client.patch(f"/scanner/rules/vendor/{rule_id}/toggle", json={"enabled": True}, headers=admin_headers)
    assert resp.status_code == 200
...
async def test_acknowledge_clean_is_idempotent(client, conn, admin_headers):
    rule_id = await _insert_vendor_rule(conn)
    await client.post(f"/scanner/rules/vendor/{rule_id}/acknowledge-clean", headers=admin_headers)
    resp2 = await client.post(f"/scanner/rules/vendor/{rule_id}/acknowledge-clean", headers=admin_headers)
...
async def test_delete_name_rule(client, conn, admin_headers):
    rule_id = await conn.fetchval(
        "INSERT INTO scanner_name_rules (disk_name, catalog_name, created_by) VALUES ($1,$2,$3) RETURNING rule_id",
        "bx_farts", "bx_farts", "admin",
    )
    resp = await client.delete(f"/scanner/rules/name/{rule_id}", headers=admin_headers)
    assert resp.status_code == 204
...
async def test_toggle_seeded_pattern_rule(client, conn, admin_headers):
    rule_id = await conn.fetchval("SELECT rule_id FROM scanner_name_patterns WHERE is_seeded=TRUE LIMIT 1")
    assert rule_id is not None
    resp = await client.patch(f"/scanner/rules/pattern/{rule_id}/toggle", json={"enabled": True}, headers=admin_headers)
    assert resp.status_code == 200
    assert resp.json()["enabled"] is True
...
async def test_create_vendor_rule_requires_auth(client):
    resp = await client.post("/scanner/rules/vendor", json={"disk_vendor": "x", "catalog_vendor": "X"})
