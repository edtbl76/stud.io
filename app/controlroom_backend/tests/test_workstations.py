...
async def test_create_workstation(client, admin_headers):
    response = await client.post("/workstations", json={"tool_name": "Test DAW"}, headers=admin_headers)
...
async def test_create_workstation_missing_name(client, admin_headers):
    response = await client.post("/workstations", json={"version": "1.0"}, headers=admin_headers)
...
async def test_update_workstation(client, conn, admin_headers):
    ...
    response = await client.patch(f"/workstations/{row['workstation_id']}", json={"version": "11.0"}, headers=admin_headers)
    assert response.status_code == 200
    assert response.json()["version"] == "11.0"


async def test_update_workstation_not_found(client, admin_headers):
    response = await client.patch(f"/workstations/{uuid4()}", json={"version": "1.0"}, headers=admin_headers)
    assert response.status_code == 404
...
async def test_delete_workstation(client, conn, admin_headers):
    ...
    response = await client.delete(f"/workstations/{row['workstation_id']}", headers=admin_headers)
    assert response.status_code == 204


async def test_delete_workstation_not_found(client, admin_headers):
    response = await client.delete(f"/workstations/{uuid4()}", headers=admin_headers)
    assert response.status_code == 404
