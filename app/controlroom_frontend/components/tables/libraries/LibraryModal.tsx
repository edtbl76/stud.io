...
interface LibraryEditFormProps {
  ...
  excludeProps: { excludeTable?: 'libraries'; excludeId?: string }
}

function LibraryEditForm({ form, set, excludeProps }: Readonly<LibraryEditFormProps>) {
...
function LibraryViewForm({ record }: Readonly<{ record: Library }>) {
  return (
    <div className="grid grid-cols-2 gap-4">
...
export function LibraryModal({ record, onClose, onMutate }: Readonly<LibraryModalProps>) {
  ...
  return (
    <RecordModal {...recordModalProps}>
      ...
      {mode === 'edit'
        ? <LibraryEditForm form={form} set={set} excludeProps={excludeProps} />
        : record && <LibraryViewForm record={record} />
      }
      </>
      )}
    </RecordModal>
