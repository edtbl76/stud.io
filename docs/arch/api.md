...
`GET /scanner/workbench[?scan_id=UUID&bucket=str&show_confirmed=bool]` — admin only. Returns all scan results for a scan with active rules applied, buckets recomputed, and rejections checked. Defaults to the most recent scan. Response: `{rows: WorkbenchRow[], scan_id}`. **Orphaned catalog records (previously the separate `orphaned` list) are merged into `rows` as `WorkbenchRow`s with `bucket='orphaned'` and empty disk fields (U-10); the `orphaned` response field was removed.** An orphaned row's `result_id` holds its `catalog_record_id`. Sorted by catalog type → catalog record name → bucket (`excluded → collision → known → needs_review → unlinked → orphaned`).

Bucket values: `excluded`, `collision`, `known`, `needs_review`, `unlinked`, `orphaned`. Orphaned rows are always returned regardless of `show_confirmed`. A **collision** (U-09) is ≥2 scan rows resolving to the same catalog record in the same format at ≥2 distinct paths; it overrides `known`/`needs_review` (FR-02) but not `excluded`. Each colliding `WorkbenchRow` carries a `collision` sub-state: `{shared_catalog_record: {id, table, name, vendor, version}, copies: [{result_id, path, version, format}]}` (the full duplicate set, used by the collision-resolution modal). Non-colliding rows have `collision: null`. Collision rows are never hidden by `show_confirmed=false`.
...
`POST /scanner/aliases` (U-19, `scanner_aliases.py`) — admin only. Body: `{disk_name, catalog_record_id, catalog_table}`. Writes one `scanner_name_aliases` row mapping the **raw** `disk_name` to the given catalog record (the "Set Name Alias" action on a single-row resolution). Validates the target first: unknown `catalog_table` or a missing/soft-deleted record → 404. **Record-aware** (mirrors acknowledge-clean): re-aliasing the same `disk_name` to the *same* record is idempotent (success, no duplicate); a `disk_name` already aliased to a *different* record → **409** (`"<disk_name> is already aliased to ..."`), never a silent overwrite. Returns the persisted alias. The alias is read on every subsequent scan with `confidence='exact'` (see `POST /scanner/scan`). This is the second direct alias-write path alongside acknowledge-clean.
