...
| Table | Description |
|---|---|
| `brands` | Companies, recording studios, and individual builders |
...
The schema lives in `sql/schema.sql`. It is applied idempotently by `app.sh` on startup (uses `CREATE TABLE IF NOT EXISTS`, `CREATE OR REPLACE VIEW`, etc.).
