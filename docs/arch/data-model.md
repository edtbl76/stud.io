...
| `plugin_scan_results` | One row per discovered plugin per scan. FK to `plugin_scans` with `ON DELETE CASCADE`. Stores raw scanned metadata (`name`, `vendor`, `version`, `format`, `path`), server-side match result (`status`, `confidence`, `score`, `record_id`, `record_table`), confirmation state (`confirmed_at`, `confirmed_by`), and soft-hide flag (`dismissed_at`). Status is the **frozen ingest-time five-bucket snapshot** (U-08 Vocabulary Frozen-Snapshot): `unlinked` (no catalog match), `needs_review` (match found but unconfirmed, field mismatch, no disk_paths, fuzzy, or collision), `known` (exact match, versions agree, catalog has disk_paths), `orphaned` (catalog record not seen in scan), `excluded` (explicitly ignored). The `status` CHECK is tightened to exactly these five values; the legacy values `matched`/`conflicted`/`unconfirmed`/`untracked`/`ignored` are removed. Existing scan-result rows (and their `plugin_scans` headers) were **wiped, not migrated**, at the cutover (FR-14); rules, links, aliases, and exclusions were preserved. No soft delete. |
