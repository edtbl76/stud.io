...
| `scanner_plugin_links` | Persistent confirmed match links — survives scan history purges. Maps a scanned plugin fingerprint (`"{vendor} {name}".lower().strip()`) to a confirmed ControlRoom catalog record. Written on `confirm`, `acknowledge`, and `force` actions; deleted on `reject`. UNIQUE on `fingerprint`. |
