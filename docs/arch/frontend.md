...
│   │   └── RuleToastManager.tsx  # fireRuleToasts — sonner toasts after rule creation (success + affected-count info); also fired from SingleResolutionModal when resolving a match creates a normalization rule (U-21)
...
│   ├── useAcknowledgeClean.ts   # Hook (U-21): Acknowledge All handler for resolution rule toasts — acknowledges a rule's clean records + invalidates the workbench query (imports the shared WORKBENCH_KEY), without loading the rules list (unlike useRules)
│   ├── useWorkbench.ts          # Hook: workbench data, client filters, sub-state derivation, selection (toggle/shift/toggleSelectAll) for ScanWorkbenchPage; exports WORKBENCH_KEY (shared query key, U-21)
