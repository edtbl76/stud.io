...
**CI check** (`./scripts/test-scan.sh --secrets`): rescans the full working tree (excluding `node_modules`, `.git`, lock files, `.next`, `__pycache__`, and `.secrets.baseline`) and compares against `.secrets.baseline`. Exits 1 if any finding is not in the baseline.
