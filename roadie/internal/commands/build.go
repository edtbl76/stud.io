...
func runSelectedSuites(ctx context.Context, cfg *config.Config, flags buildFlags, out io.Writer) error {
	r := pipeline.Root(".")
	if err := maybeRunE2E(ctx, cfg, flags, r, out); err != nil {
		return err
	}
...
func maybeRunE2E(ctx context.Context, cfg *config.Config, flags buildFlags, r pipeline.Root, out io.Writer) error {
	if !flags.runE2E() {
		return nil
	}
	fmt.Fprintln(out, "[roadie] Running E2E tests...")
	return pipeline.RunE2E(ctx, e2eConfigFrom(cfg), r, out)
}
