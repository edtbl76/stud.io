...
func setupBackendShards(ctx context.Context, cfg E2EConfig, root string, out io.Writer) error {
	if err := buildBackendImage(ctx, cfg, root, out); err != nil {
		return err
	}
	if err := stopDevBackend(ctx, cfg, root, out); err != nil {
		return err
	}
...
// runDevCompose runs a docker compose command using cfg's compose file and project.
func runDevCompose(ctx context.Context, cfg E2EConfig, root string, out io.Writer, verb string, extra ...string) error {
	args := append([]string{
		"compose",
...
func buildBackendImage(ctx context.Context, cfg E2EConfig, root string, out io.Writer) error {
	...
	return runDevCompose(ctx, cfg, root, out, "build", cfg.BackendService)
}
...
func stopDevBackend(ctx context.Context, cfg E2EConfig, root string, out io.Writer) error {
	...
	return runDevCompose(ctx, cfg, root, out, "stop", cfg.BackendService)
}

func restoreDevBackend(cfg E2EConfig, root string) {
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	runDevCompose(ctx, cfg, root, io.Discard, "up", "-d", cfg.BackendService) //nolint — best-effort
}
